...
class WPCAC_AdminPageFramework_View__PageMetaboxEnabler extends WPCAC_AdminPageFramework_FrameworkUtility {
    public $oFactory;
    ...
    {
        $_aPageMetaBoxClasses = $this->getElementAsArray($GLOBALS, array( 'aWPCAC_AdminPageFramework', 'aMetaBoxForPagesClasses' ));
    ...
    {
        $_bLoaded = $this->getElement($GLOBALS, array( 'aWPCAC_AdminPageFramework', 'bAddedMetaBoxScript' ), false);
        if ($_bLoaded) {
            return;
        }
        $GLOBALS[ 'aWPCAC_AdminPageFramework' ][ 'bAddedMetaBoxScript' ] = true;
        $_sScript = <<<JAVASCRIPTS
jQuery(document).ready(function(){postboxes.add_postbox_toggles(pagenow)})
JAVASCRIPTS;
        echo '<script class="wpcommand-insert-metabox-script">' . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . '</script>';
    }
}
