...
class WPCAC_AdminPageFramework_Model__FormSubmission__Validator__Filter extends WPCAC_AdminPageFramework_Model__FormSubmission_Base {
    public $oFactory;
    ...
    public function get()
    {
        return $this->___getFiltered($this->aInputs, $this->aRawInputs, $this->aOptions, $this->aSubmitInformation);
    }
    private function ___getFiltered($aInputs, $aRawInputs, $aStoredData, $aSubmitInformation)
    {
        ...
        $_aData = $this->___validateEachField($_aData, $aRawInputs);
        $_aData = $this->___validateTabFields($_aData);
        $_aData = $this->___validatePageFields($_aData);
        $_aInput = $this->___getValidatedData("validation_{$this->oFactory->oProp->sClassName}", call_user_func_array(array( $this->oFactory, 'validate' ), array( $_aData[ 'aInput' ], $_aData[ 'aStoredData' ], $this->oFactory, $_aData[ 'aSubmitInformation' ] )), $_aData[ 'aStoredData' ], $_aData[ 'aSubmitInformation' ]);
        ...
        foreach ($aInputsToParse as $_sID => $_aSectionOrFields) {
            if ($this->oFactory->oForm->isSection($_sID)) {
                ...
                foreach ($_aSectionOrFields as $_sFieldID => $_aFields) {
                    $aData[ 'aInput' ][ $_sID ][ $_sFieldID ] = $this->___getValidatedData("validation_{$this->oFactory->oProp->sClassName}_{$_sID}_{$_sFieldID}", $aData[ 'aInput' ][ $_sID ][ $_sFieldID ], $this->getElement($aData, array( 'aStoredData', $_sID, $_sFieldID ), null), $aData[ 'aSubmitInformation' ]);
                }
                ...
                $aData[ 'aInput' ][ $_sID ] = $this->___getValidatedData("validation_{$this->oFactory->oProp->sClassName}_{$_sID}", $_aSectionInput, $this->getElement($aData, array( 'aStoredData', $_sID ), null), $aData[ 'aSubmitInformation' ]);
                continue;
            }
            if (! $this->___isValidSection('_default', $aData[ 'sPageSlug' ], $aData[ 'sTabSlug' ])) {
                continue;
            }
            $aData[ 'aInput' ][ $_sID ] = $this->___getValidatedData("validation_{$this->oFactory->oProp->sClassName}_{$_sID}", $aData[ 'aInput' ][ $_sID ], $this->getElement($aData, array( 'aStoredData', $_sID ), null), $aData[ 'aSubmitInformation' ]);
        }
        return $aData;
    }
    private function ___isValidSection($sSectionID, $sPageSlug, $sTabSlug)
    {
        if ($sPageSlug && isset($this->oFactory->oForm->aSectionsets[ $sSectionID ][ 'page_slug' ]) && $sPageSlug !== $this->oFactory->oForm->aSectionsets[ $sSectionID ][ 'page_slug' ]) {
            return false;
        }
        if ($sTabSlug && isset($this->oFactory->oForm->aSectionsets[ $sSectionID ][ 'tab_slug' ]) && $sTabSlug !== $this->oFactory->oForm->aSectionsets[ $sSectionID ][ 'tab_slug' ]) {
            return false;
        }
        return true;
    }
    private function ___validateTabFields(array $aData)
    {
        ...
        $aData[ 'aInput' ] = $this->___getValidatedData("validation_{$aData[ 'sPageSlug' ]}_{$aData[ 'sTabSlug' ]}", $aData[ 'aInput' ], $aData[ 'aStoredTabData' ], $aData[ 'aSubmitInformation' ]);
        $aData[ 'aEmbeddedDataWODynamicElements' ] = $this->___getEmbeddedOptions($aData[ 'aInput' ], $aData[ 'aStoredTabDataWODynamicElements' ], $_aTabOnlyOptionsWODynamicElements);
    ...
    private function ___validatePageFields(array $aData)
    {
        ...
        $aData[ 'aInput' ] = $this->___getValidatedData("validation_{$aData[ 'sPageSlug' ]}", $aData[ 'aInput' ], $_aPageOptions, $aData[ 'aSubmitInformation' ]);
        ...
        $_aEmbeddedOptionsWODynamicElements = $aData[ 'aEmbeddedDataWODynamicElements' ] + $this->___getEmbeddedOptions($aData[ 'aInput' ], $_aPageOptionsWODynamicElements, $_aPageOnlyOptionsWODynamicElements);
        $aData[ 'aInput' ] = $aData[ 'aInput' ] + $this->uniteArrays($_aPageOptions, $_aOtherPageOptions, $_aEmbeddedOptionsWODynamicElements);
        return $aData;
    }
    private function ___getEmbeddedOptions(array $aInputs, array $aOptions, array $aPageSpecificOptions)
    {
