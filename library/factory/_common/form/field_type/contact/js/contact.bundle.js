...
(function($){
  ...
  var apfContact = WPCAC_AdminPageFrameworkContactFieldType;
  ...
  function sendEmail( event ) {
    ...
    // Check required
    var _requiredInputs = $( this ).closest( '.wpcommand-sections' )
    ...
    var _oSpinner = $('<img src="' + apfMain.spinnerURL + '" alt="' + apfMain.messages.loading + '" />' )
      .addClass( 'wpcommand-ajax-spinner' );
    $( this ).closest( '.wpcommand-field' ).find( '.result-placeholder' ).prepend( _oSpinner );
    var _resultIcon = $( this ).closest( '.wpcommand-field' ).find( '.result-placeholder .dashicons' );
    ...
    $.ajax( {
        ...
        success: function ( response ) {
          ...
          if ( response.result ) {
            _resultIcon.addClass( 'dashicons-yes-alt success wpcommand-form-tooltip' );
            _resultIcon.prepend( "<span class='wpcommand-form-tooltip-content'>" + response.message + "</span>" );
          } else {
            _resultIcon.addClass( 'dashicons-warning error wpcommand-form-tooltip' );
            _resultIcon.prepend( "<span class='wpcommand-form-tooltip-content'>" + response.message + "</span>" );
          }
        },
        error: function( response ) {
          ...
          _resultIcon.addClass( 'dashicons-warning error wpcommand-form-tooltip' );
          _resultIcon.prepend( "<span class='wpcommand-form-tooltip-content'>" + response.status + ' ' + response.statusText + "</span>" );
        },
        complete: function() {
          _oSpinner.remove();
          debugLog( 'Request done.' );
          _resultIcon[ 'wpcommand-form-tooltip' ]({
              autoClose: false,
              shown: true,
              oneOff: true,
              noArrow: true,
