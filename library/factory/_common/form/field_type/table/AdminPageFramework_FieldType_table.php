...
class WPCAC_AdminPageFramework_FieldType_table extends WPCAC_AdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'wpcommand-field-type-table', 'src' => dirname(__FILE__) . '/js/table.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'jquery-ui-accordion', 'wpcommand-script-form-main' ), 'translation_var' => 'WPCAC_AdminPageFrameworkFieldTypeTable', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'label' => array(), ), ), );
    }
    ...
    private function ___getLabel($aField)
    {
        ...
        return "<div class='wpcommand-input-label-container'>" . "<label for='" . esc_attr($aField[ 'input_id' ]) . "'>" . "<span " . $this->getLabelContainerAttributes($aField, 'wpcommand-input-label-string') . ">" . $aField[ 'label' ] . "</span>" . "</label>" . "</div>";
    }
    private function ___getTable($aField)
    {
        ...
        $_sCaption = $aField[ 'caption' ] ? $aField[ 'caption' ] : __('Set the caption with the <code>caption</code> argument.', 'wpcommand');
