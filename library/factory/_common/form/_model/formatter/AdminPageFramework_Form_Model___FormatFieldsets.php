...
class WPCAC_AdminPageFramework_Form_Model___FormatFieldsets extends WPCAC_AdminPageFramework_Form_Base {
    ...
    public function get()
    {
        $this->aFieldsets = $this->___getFieldsetsFormatted($this->aFieldsets, $this->aSectionsets, $this->sCapability);
        return $this->___getDynamicElementsAddedToFieldsets();
    }
    private function ___getDynamicElementsAddedToFieldsets()
    {
        $_oDynamicElements = new WPCAC_AdminPageFramework_Form_Model___FormatDynamicElements($this->aSectionsets, $this->aFieldsets, $this->aSavedData);
        return $_oDynamicElements->get();
    }
    private function ___getFieldsetsFormatted(array $aFieldsets, array $aSectionsets, $sCapability)
    {
        ...
        foreach ($aFieldsets as $_sSectionPath => $_aItems) {
            ...
            $_aNewFieldsets[ $_sSectionPath ] = $this->___getItemsFormatted($_sSectionPath, $_aItems, $this->getElement($aSectionsets, array( $_sSectionPath, 'capability', ), $sCapability), $aSectionsets);
        }
        $this->___sortFieldsBySectionsOrder($_aNewFieldsets, $aSectionsets);
        return $_aNewFieldsets;
    }
    private function ___getItemsFormatted($sSectionPath, $aItems, $sCapability, $aSectionsets)
    {
        $_abSectionRepeatable = $this->getElement($aSectionsets, array( $sSectionPath, 'repeatable' ), false);
        if ($this->___isSubSections($aItems, $_abSectionRepeatable)) {
            return $this->___getSubSectionsFormatted($aItems, $sCapability, $aSectionsets, $_abSectionRepeatable);
        }
        return $this->___getNormalFieldsetsFormatted($aItems, $sCapability, $aSectionsets, $_abSectionRepeatable);
    }
    private function ___getNormalFieldsetsFormatted($aItems, $sCapability, $aSectionsets, $_abSectionRepeatable)
    {
        ...
        foreach ($aItems as $_sFieldID => $_aFieldset) {
            $_aFieldset = $this->___getFieldsetFormatted($_aFieldset, $aSectionsets, $sCapability, count($_aNewItems), null, $_abSectionRepeatable, $this->oCallerForm);
            if (empty($_aFieldset)) {
                continue;
            }
    ...
    private function ___getSubSectionsFormatted($aItems, $sCapability, $aSectionsets, $_abSectionRepeatable)
    {
        ...
        foreach ($this->numerizeElements($aItems) as $_iSubSectionIndex => $_aFieldsets) {
            foreach ($_aFieldsets as $_aFieldset) {
                ...
                $_aFieldset = $this->___getFieldsetFormatted($_aFieldset, $aSectionsets, $sCapability, $_iCountElement, $_iSubSectionIndex, $_abSectionRepeatable, $this->oCallerForm);
                if (empty($_aFieldset)) {
                    continue;
                }
    ...
    private function ___sortFieldsBySectionsOrder(array &$aFieldsets, array $aSectionsets)
    {
    ...
    private function ___getFieldsetFormatted($aFieldset, $aSectionsets, $sCapability, $iCountOfElements, $iSubSectionIndex, $bIsSectionRepeatable, $oCallerObject)
    {
        ...
        $_oFieldsetFormatter = new WPCAC_AdminPageFramework_Form_Model___Format_Fieldset($aFieldset, $this->sStructureType, $sCapability, $iCountOfElements, $iSubSectionIndex, $bIsSectionRepeatable, $oCallerObject);
