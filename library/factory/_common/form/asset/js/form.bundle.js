...
(function ( $ ) {
     ...
     */
    $.fn.selectAllWPCAC_AdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
     ...
     */
    $.fn.deselectAllWPCAC_AdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
            .find( 'input[type=checkbox]' )
            .prop( 'checked', false )
            .trigger( 'change' );   // 3.8.8+
...
( function( $ ) {
    ...
    var _removeWPCAC_AdminPageFrameworkLoadingOutputs = function() {

        jQuery( '.wpcommand-form-loading' ).remove();
        jQuery( '.wpcommand-form-js-on' )
            .hide()
            .css( 'visibility', 'visible' )
            .fadeIn( 200 )
            .removeClass( '.wpcommand-form-js-on' );
    ...
    window.onerror = function(){
        ...
        _removeWPCAC_AdminPageFrameworkLoadingOutputs();
...
(function ( $ ) {
    ...
    $.fn.aWPCAC_AdminPageFrameworkInputOptions = {};

    $.fn.storeWPCAC_AdminPageFrameworkInputOptions = function( sID, vOptions ) {
        ...
        $.fn.aWPCAC_AdminPageFrameworkInputOptions[ sID ] = vOptions;
    };
    $.fn.getWPCAC_AdminPageFrameworkInputOptions = function( sID ) {
        sID = sID.replace( /__\d+_/, '___' ); // remove the section index
        return ( 'undefined' === typeof $.fn.aWPCAC_AdminPageFrameworkInputOptions[ sID ] )
            ? null
            : $.fn.aWPCAC_AdminPageFrameworkInputOptions[ sID ];
    }
...
(function ( $ ) {
    ...
    // Callback containers.
    $.fn.aWPCAC_AdminPageFrameworkAddRepeatableFieldCallbacks        = [];
    $.fn.aWPCAC_AdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    $.fn.aWPCAC_AdminPageFrameworkRemoveRepeatableFieldCallbacks     = [];
    $.fn.aWPCAC_AdminPageFrameworkSortedFieldsCallbacks              = [];
    $.fn.aWPCAC_AdminPageFrameworkStoppedSortingFieldsCallbacks      = [];
    $.fn.aWPCAC_AdminPageFrameworkAddedWidgetCallbacks               = [];
    $.fn.aWPCAC_AdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+

    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */
    $( document ).on( 'wpcommand_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aWPCAC_AdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registerWPCAC_AdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     *
     * Unlike the `wpcommand_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).on( 'wpcommand_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aWPCAC_AdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).on( 'wpcommand_stopped_sorting_sections', function( oEvent ){

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aWPCAC_AdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).on( 'wpcommand_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aWPCAC_AdminPageFrameworkRemoveRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $.fn.callBackSortedFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aWPCAC_AdminPageFrameworkSortedFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
    ...
    $.fn.callBackStoppedSortingFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aWPCAC_AdminPageFrameworkStoppedSortingFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
     ...
     */
    $( document ).on( 'wpcommand_saved_widget', function( event, oWidget ){
        $.each( $.fn.aWPCAC_AdminPageFrameworkAddedWidgetCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 'function' !== typeof _hfCallback ) {
                return true;    // continue
     ...
     */
    $.fn.registerWPCAC_AdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aWPCAC_AdminPageFrameworkAddRepeatableFieldCallbacks.push(
            [ oCallbacks.added_repeatable_field, aFieldTypeSlugs ]
        );

        $.fn.aWPCAC_AdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
            [ oCallbacks.repeated_field, aFieldTypeSlugs ]
        );
        $.fn.aWPCAC_AdminPageFrameworkRemoveRepeatableFieldCallbacks.push(
            [ oCallbacks.removed_repeatable_field, aFieldTypeSlugs ]
        );
        $.fn.aWPCAC_AdminPageFrameworkSortedFieldsCallbacks.push(
            [ oCallbacks.sorted_fields, aFieldTypeSlugs ]
        );
        $.fn.aWPCAC_AdminPageFrameworkStoppedSortingFieldsCallbacks.push(
            [ oCallbacks.stopped_sorting_fields, aFieldTypeSlugs ]
        );
        $.fn.aWPCAC_AdminPageFrameworkAddedWidgetCallbacks.push(
            [ oCallbacks.saved_widget, aFieldTypeSlugs ]
        );

        // 3.8.0
        $.fn.aWPCAC_AdminPageFrameworkStoppedSortingSectionsCallbacks.push(
            [ oCallbacks.stopped_sorting_sections, aFieldTypeSlugs ]
        );

    };
    /**
     * An alias of the `registerWPCAC_AdminPageFrameworkCalbacks()` method.
    ...
    $.fn.registerAPFCallback = function( oCallbacks, aFieldTypeSlugs ) {
        $.fn.registerWPCAC_AdminPageFrameworkCallbacks( oCallbacks, aFieldTypeSlugs );
    }
...
(function ( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.wpcommand-fields.repeatable' ).each( function() {
            var _buttonModel = $( this ).siblings( '.repeatable-field-buttons-model' );
            if ( ! _buttonModel.length ) {
                return true;
            }
            var _aSettings           = _buttonModel.data();
            var _buttonContainer     = _buttonModel.children( '.wpcommand-repeatable-field-buttons' ).first();
            ...
            var _childFields = $( this ).find( '> .wpcommand-field.without-child-fields' );
            ...
            /* Otherwise, insert the button element at the beginning of the field tag */
                 ...
                 */
                if ( ! _childFields.find( '.wpcommand-repeatable-field-buttons' ).length ) {
             ...
             */
            $( this ).find( '> .wpcommand-field.with-child-fields' ).prepend( _buttonsNestedFields );
                /**
                 * Add buttons to the fields tag.
                 */
                // $( this ).find( '.wpcommand-field.with-nested-fields' ).prepend( _buttonsNestedFields );

                /**
                 * Support for inline mixed fields.
                 * @todo not sure why this is commented out. Remove this if it's okay.
                 */
                // $( this ).find( '.wpcommand-field.with-mixed-fields' ).prepend( _buttonsNestedFields );




            $( this ).updateWPCAC_AdminPageFrameworkRepeatableFields( _aSettings ); // Update the fields
     ...
     * @var WPCAC_AdminPageFrameworkScriptFormMain
     */
    var translation = WPCAC_AdminPageFrameworkScriptFormMain;
     ...
     */
    $.fn.updateWPCAC_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.wpcommand-repeatable-field-buttons' )
            .filter( function() {
                return $( this ).closest( '.wpcommand-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            });
        ...
        _oRepeatableAddButtons.on( 'click', function() {
            ...
            $( this ).addWPCAC_AdminPageFrameworkRepeatableField();
        ...
        _oRepeatableRemoveButton.on( 'click', function() {
            ...
            $( this ).removeWPCAC_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.wpcommand-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.wpcommand-field' )
            .filter( function() {
                return $( this ).closest( '.wpcommand-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            })
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {
                $( '#' + _sFieldID ).addWPCAC_AdminPageFrameworkRepeatableField( _sFieldID );
            }
        }
     ...
     */
    $.fn.addWPCAC_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {

        if ( 'undefined' === typeof sFieldContainerID ) {
            sFieldContainerID = $( this ).closest( '.wpcommand-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.wpcommand-fields' );
        ...
        if ( ! _aOptions ) {
            var _nodeButtonContainer = nodeFieldContainer.find( '.wpcommand-repeatable-field-buttons' )
                .filter( function() {
                    return $( this ).closest( '.wpcommand-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.wpcommand-field' )
                                    .filter( function() {
                                        return $( this ).closest( '.wpcommand-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.wpcommand-repeatable-field-buttons' )
                                    .filter( function() {
                                        return $( this ).closest( '.wpcommand-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
         ...
         */
        nodeNewField.updateWPCAC_AdminPageFrameworkRepeatableFields();
        ...
        nodeNewField.trigger(
            'wpcommand_added_repeatable_field',
            [
        ...
        $( nodeNewField ).find( '.wpcommand-field' ).addBack().trigger(
            'wpcommand_repeated_field',
            [
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.wpcommand-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {
                return $( this ).closest( '.wpcommand-fields' ).attr( 'id' ) === _sFieldsContainerID;
            })
        ...
        nodeFieldsContainer.children( '.wpcommand-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.wpcommand-field,.wpcommand-fields,.wpcommand-fieldset' ).incrementAttributes(
                [ 'id', 'data-field_tag_id_model', 'data-field_id' ],
                _iFieldCount,
                _sFieldTagIDModel
            );
            oElement.find( '.wpcommand-fields' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeWPCAC_AdminPageFrameworkRepeatableField = function() {

        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.wpcommand-field' );
        var nodeFieldsContainer = $( this ).closest( '.wpcommand-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
...
( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.wpcommand-sections.repeatable-section' ).each( function( index ){

            var _buttonModel    = $( this ).children( '.repeatable-section-buttons-model' ).first();
            var _repeatOptions  = _buttonModel.data();
            _buttonModel = _buttonModel.find( '.wpcommand-repeatable-section-buttons' ).first().detach();

            // Add buttons
            $( this ).find( '.wpcommand-section-caption' ).each( function(){

                $( this ).show();
                var _oButtons = $( _buttonModel[0].outerHTML );
                if ( $( this ).children( '.wpcommand-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
                    _oButtons.addClass( 'section_title_field_sibling' );
                }
                var _oCollapsibleSectionTitle = $( this ).find( '.wpcommand-collapsible-section-title' );
            ...
            // Update the fields
            $( this ).updateWPCAC_AdminPageFrameworkRepeatableSections( _repeatOptions );
     ...
     */
    $.fn.updateWPCAC_AdminPageFrameworkRepeatableSections = function( aSettings ) {

        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.wpcommand-sections' ).attr( 'id' );

        // Store the sections specific options in an array.
        if ( ! $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {
        ...
        var _aOptions = $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).on( 'click', function() {
            ...
            $( this ).addWPCAC_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // The Remove button behavior
        $( _oThis ).find( '.repeatable-section-remove-button' ).on( 'click', function() {
            $( this ).removeWPCAC_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // If the number of sections is less than the set minimum value, add sections.
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.wpcommand-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.wpcommand-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {
                $( '#' + _sSectionID ).addWPCAC_AdminPageFrameworkRepeatableSection( _sSectionID );
            }
        }
     ...
     */
    $.fn.addWPCAC_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {

        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            sSectionContainerID = $( this ).closest( '.wpcommand-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.wpcommand-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.wpcommand-section-tabs-contents' )
            .children( '.wpcommand-section-tabs' )
            .first();

        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );

        var _iFadein                = $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        var _bPreserveValues        = $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'preserve_values' ];

        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.wpcommand-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.wpcommand-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.wpcommand-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.wpcommand-section-title' ).not( '.wpcommand-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableWPCAC_AdminPageFrameworkCollapsibleButton ){
            nodeNewSection.find( '.wpcommand-collapsible-sections-title, .wpcommand-collapsible-section-title' ).enableWPCAC_AdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.wpcommand-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updateWPCAC_AdminPageFrameworkRepeatableFields();
            ...
            $( this ).trigger(
                ...
                [
                    ...
                    jQuery( nodeNewSection ).closest( '.wpcommand-sections' )    // model container
                ]
            );
        ...
        nodeNewSection.updateWPCAC_AdminPageFrameworkRepeatableSections();

        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.wpcommand-fields.sortable' ).each( function() {
            $( this ).enableWPCAC_AdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.wpcommand-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.wpcommand-fieldrow, .wpcommand-fieldset, .wpcommand-fields, .wpcommand-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute(
                    'id',
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.wpcommand-fields' ).incrementAttribute(
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.wpcommand-fieldset' ).incrementAttribute(
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removeWPCAC_AdminPageFrameworkRepeatableSection = function() {

        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.wpcommand-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.wpcommand-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.wpcommand-section-tabs-contents' )
            .children( '.wpcommand-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.wpcommand-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.aWPCAC_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.wpcommand-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.wpcommand-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() {
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.wpcommand-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.wpcommand-section-title' ).first().show();
                }
            }
        ...
        _oNextAllSections.each( function( _iIterationIndex ) {
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.wpcommand-field' ).each( function( iFieldIndex ) {
...
( function( $ ) {
  ...
  $( document ).ready( function() {
    ...
    $( '.wpcommand-section-tabs-contents' ).createTabs();
  });
...
(function($) {
    ...
    // Initialize
    $( document ).ready( function() {
        $( '.wpcommand-fields.sortable' ).each( function() {
            $( this ).enableWPCAC_AdminPageFrameworkSortableFields();
        } );
    });

    $.fn.enableWPCAC_AdminPageFrameworkSortableFields = function( sFieldsContainerID ) {
...
(function($) {
    ...
    // Initialization
    $( document ).ready( function() {
        $( '.wpcommand-sections.sortable-section' ).each( function(){
            $( this ).enableWPCAC_AdminPageFrameworkSortableSections();
        } );
    });

    $.fn.enableWPCAC_AdminPageFrameworkSortableSections = function( sSectionsContainerID ) {
        ...
        var _bIsTabbed      = _oTarget.hasClass( 'wpcommand-section-tabs-contents' );
        var _bCollapsible   = 0 < _oTarget.children( '.wpcommand-section.is_subsection_collapsible' ).length;

        _oTarget        = _bIsTabbed
            ? _oTarget.find( 'ul.wpcommand-section-tabs' )
            : _oTarget;
        ...
        var _aSortableOptions = {
                items: _bIsTabbed
                    ? '> li:not( .disabled )'
                    : '.wpcommand-section:not( .disabled, .wpcommand-collapsible-toggle-all-button-container )',
                handle: _bCollapsible
                    ? '.wpcommand-section-caption'
                    : false,
                ...
                stop: function(e,ui) {
                    ...
                    // Callback the registered callback functions.
                    $( this ).trigger(
                        'wpcommand_stopped_sorting_sections',
                ...
                // cancel: '.wpcommand-section-description, .wpcommand-section-title'
        ...
        if ( ! _bIsTabbed ) {
            ...
            _oSortable.on( 'sortstop', function() {
                ...
                $( this ).find( 'caption > .wpcommand-section-title:not(.wpcommand-collapsible-sections-title,.wpcommand-collapsible-section-title)' ).first().show();
                $( this ).find( 'caption > .wpcommand-section-title:not(.wpcommand-collapsible-sections-title,.wpcommand-collapsible-section-title)' ).not( ':first' ).hide();
  ...
  function debugLog( ...message ) {
    if ( ! parseInt( WPCAC_AdminPageFrameworkScriptFormMain.debugMode ) ) {
      return;
    }
...
(function($){
  ...
  $.widget( 'wpcommand.pointerTooltip', $.wp.pointer, {
   /**
 ...
 *
 * It provides the $[ 'wpcommand-tooltip' ]( options ) jQuery plugin method.
 * The name uses hyphens for the user's text domain to be replaced. So the compiled framework script will be entirely ported with without the keyword of `wpcommand`.
 *
 * To use the method,
 * ```
 * $( '.my-tooltip' )[ 'wpcommand-form-tooltip' ]( 'Hi, this is a tooltip content!' );
 * ```
 * Or have a text message enclosed in an element with a class `wpcommand-form-tooltip-content`.
 * ```
 *    <span class="my-tooltip dashicons dashicons-editor-help">
 *      <span class="wpcommand-form-tooltip-content">
 *        Some text
 *      </span>
 *    </span>
 * ```
 * Then, call it like
 * ```
 * $( '.my-tooltip' )[ 'wpcommand-form-tooltip' ]();
 * ```
 * Or
 * ```
 ...
 * ```
 *
 * If it the script is loaded, elements with the .wpcommand-from-tooltip class will be automatically parsed.
 * So not to call the `[ 'wpcommand-form-tooltip' ]()` method, just create an element with the selector
 * and it should automatically have a tooltip.
 *
 *
 * When the framework file is compiled, replace the keyword `wpcommand` with your text domain.
 */
(function($){

  // Initialize
  $( document ).ready( function() {
    $( '.wpcommand-form-tooltip' ).each( function() {
      $( this )[ 'wpcommand-form-tooltip' ]();
    } );
  } );

  $.fn[ 'wpcommand-form-tooltip' ] = function( options ) {
  ...
  function initialize( target, options ) {
    ...
    options = $.extend( {}, {
      pointerClass: 'wpcommand-form-tooltip-balloon',
  ...
  function handleTooltip( self, options ) {
    ...
    var _content   = 'undefined' !== typeof options.content
      ...
      : $( self ).find( '.wpcommand-form-tooltip-content' ).clone();
    ...
    var _options    = $.extend( true, {}, options, {
      ...
      position: {
        ...
        within: _offscreen ? _body : $( self ).closest( '.wpcommand-field, .wpcommand-fieldrow, .wpcommand-section' ),
      },
      buttons: function() {},
      close: function() {},
    }, options );
  ...
  function handleAutoClose( self, content ) {
      ...
      $( self ).add( '.wpcommand-form-tooltip-balloon' ).on( 'mouseleave', function( event ){
      ...
      $( self ).add( '.wpcommand-form-tooltip-balloon' ).on( 'mouseenter', function(){
    ...
    $( document ).ready( function() {
        ...
        $( document ).ajaxComplete( function( event, XMLHttpRequest, ajaxOptions ) {
            ...
            if( _aRequest.action && ( 'save-widget' === _aRequest.action ) ) {
                ...
                if ( $( _oWidget ).find( '.wpcommand-sectionset' ).length <= 0 ) {
                    return;
                }
