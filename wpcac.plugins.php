...
function _wpcac_upgrade_plugin( $plugin ) {
    ...
    if ( $is_active ) {
        ...
        $req_body = array( 'wpcac_api_key' => $_POST['wpcac_api_key'], 'actions' => 'activate_plugin', 'plugin' => $plugin  );
        $verify_key = _WPCAC_generate_hashes($req_body);
        $req_body['wpcac_verify'] = $verify_key[0];
...
function _wpcac_install_plugin( $plugin, $args = array() ) {
    ...
    //$api = plugins_api( 'plugin_information', array( 'slug' => 'custom-favicon' ) );
 ...
 */
function _wpcac_uninstall_plugin( $plugin ) {
    ...
    if ( ! _wpcac_check_filesystem_access() || ! WP_Filesystem() )
        return array( 'status' => 'error', 'error' => 'The filesystem is not writable with the supplied credentials' );
...
function _wpcac_add_non_extend_plugin_support_filter() {
    add_filter( 'pre_set_site_transient_update_plugins', '_wpcac_add_non_extend_plugin_support' );
}

function _wpcac_add_non_extend_plugin_support( $value ) {

    foreach( $non_extend_list = _wpcac_get_non_extend_plugins_data() as $key => $anon_function ) {
...
function _wpcac_get_non_extend_plugins_data() {

    return array(
        'gravity_forms' => '_wpcac_get_gravity_form_plugin_data',
        'backupbuddy' => '_wpcac_get_backupbuddy_plugin_data',
        'tribe_events_pro' => '_wpcac_get_tribe_events_pro_plugin_data'
    );

}

function _wpcac_get_gravity_form_plugin_data() {
