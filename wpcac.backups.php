 ...
 */
class WPCAC_Backups extends WPCAC_HM_Backup {
    ...
    /**
     * Return the current instance of WPCAC_Backups
     *
     * @static
     * @access public
     */
    public static function get_instance() {
        ...
        if ( empty( self::$instance ) )
            self::$instance = new WPCAC_Backups();
...
endswitch;
        ...
        private function path() {
            ...
            $path = get_option( 'wpcac_backup_path' );
            ...
            if ( get_option( 'wpcac_backup_path' ) !== $path )
                update_option( 'wpcac_backup_path', $path );
            ...
            if ( $is_apache && function_exists( 'insert_with_markers' ) && ! file_exists( $htaccess ) && is_writable( $path ) ) {
                ...
                $contents[]	= '# ' . sprintf( __( 'This %s file ensures that other people cannot download your backup files.', 'wpcac' ), '.htaccess' );
                $contents[] = '';
                $contents[] = '<IfModule mod_rewrite.c>';
                $contents[] = 'RewriteEngine On';
                $contents[] = 'RewriteCond %{QUERY_STRING} !key=' . WPCAC_SECURE_KEY;
