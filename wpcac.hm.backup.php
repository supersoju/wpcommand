...
class WPCAC_HM_Backup {
	...
	public function archive_sql() {
		
		$this->get_type = 'database';
	...
	public function pcl_zip() {
		...
		global $_wpcac_hmbkp_exclude_string;

		$_wpcac_hmbkp_exclude_string = $this->exclude_string( 'regex' );
		...
		// Zip up everything
		if ( $this->get_type() !== 'database' )
			if ( ! $archive->add( $this->get_root(), PCLZIP_OPT_REMOVE_PATH, $this->get_root(), PCLZIP_CB_PRE_ADD, 'wpcac_hmbkp_pclzip_callback' ) )
		...
		unset( $GLOBALS['_wpcac_hmbkp_exclude_string'] );
 ...
 * @return bool
 */
function wpcac_hmbkp_pclzip_callback( $event, &$file ) {
    ...
    elseif ( $_wpcac_hmbkp_exclude_string && preg_match( '(' . $_wpcac_hmbkp_exclude_string . ')', $file['stored_filename'] ) )
    	return false;
